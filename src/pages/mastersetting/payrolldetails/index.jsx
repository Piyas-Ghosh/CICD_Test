...
function App() {
	...
	const handleSubmit = (e) => {
		...
		if (type === 'earning') {
			...
			fetch(`${import.meta.env.VITE_API_URL}/company-components-write`, {
				method: 'POST',
				body: JSON.stringify({
					...formData,
					marking: isChecked,
			...
			if (ruleId === '1') {
				...
				// Perform API call with formData
				fetch(
					`${import.meta.env.VITE_API_URL}/company-components-write`,
					{
						method: 'POST',
						body: JSON.stringify({
							...esicComponent1,
		...
		const companyComponents = async () => {
			try {
				const response1 = await fetch(
					`${import.meta.env.VITE_API_URL}/company-components-read`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
