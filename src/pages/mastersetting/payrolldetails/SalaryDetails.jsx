...
const SalaryDetails = () => {
	...
	useEffect(() => {
		...
		fetch(
			`${import.meta.env.VITE_API_URL}/salary-details-value/${empid}`,
			{
				headers: {
	...
	const handleCheckboxClickPtax = async () => {
		...
		try {
			const response1 = await fetch(
				`${import.meta.env.VITE_API_URL}/rule-two/${CTC}/${user.id}`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const handleCheckboxClickPF = async () => {
		...
		try {
			const response1 = await fetch(
				`${import.meta.env.VITE_API_URL}/rule-three/${basic}`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const handleCheckboxClickESIC = async () => {
		...
		try {
			const response1 = await fetch(
				`${import.meta.env.VITE_API_URL}/rule-one/${CTC}`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const salaryDetailsSubmit = async (e) => {
		...
		try {
			setIsLoading(true);
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/salary-details-submit`,
				{
					method: 'POST',
					body: formData,
					headers: {
