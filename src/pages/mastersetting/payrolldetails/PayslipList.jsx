		...
		const departmentList = async () => {
			try {
				...
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/payslip-list/${empid}`,
					{
						method: 'GET',
						headers: {
		...
		const generatePaySlip = async (e) => {
			...
			try {
				...
				const response1 = await fetch(
					`${import.meta.env.VITE_API_URL}/generate-payslip`,
					{
						method: 'POST',
						body: formData,
						headers: {
	...
	const ViewPaySlip = (rowData) => {
		const payslip = async () => {
			...
			try {
				setLoader(true);
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/view-payslip/${rowData.payslip_unique_id}`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
						...
						<ModalBody>
							<Box width='100%' margin='20px auto 0px'>
								<Box
									...
									<Box>
										<Heading mb="10px">
											{payslipData.company_name === 'null' || payslipData.company_name === 'undefined'
												? ''
												: payslipData.company_name}
										</Heading>
										<Text fontWeight="600" mb="5px">
											{payslipData.company_address === 'null' || payslipData.company_address === 'undefined'
												? ''
							...
							<Box p='10px 0px'>
								<Box>
									<Text fontWeight='600' mb='5px'>
										{payslipData.payment_date === 'null' || payslipData.payment_date ===
											'undefined'
											? ''
											: payslipData.payment_date}
									</Text>
									<Text fontWeight='600' mb='5px'>
										{payslipData.emp_name === 'null' || payslipData.emp_name ===
											'undefined'
											? ''
											: payslipData.emp_name}
										{payslipData.emp_code === 'null' || payslipData.emp_code ===
											'undefined'
											? ''
									...
									<Text mb="10px">
										...
										{payslipData.date_of_joining === 'null' || payslipData.date_of_joining === 'undefined'
											? ''
								...
								<Box display='flex' m='50px 0px 20px'>
									<Box w='25%'>
										...
										<Text>
											{payslipData.uan === 'null' || payslipData.uan ===
												'undefined'
												? 'No UAN Number'
												: payslipData.uan}
										</Text>
									...
									<Box w='25%'>
										...
										<Text>
											{payslipData.pf === 'null' || payslipData.pf === 'undefined'
												? 'No Pf Number'
												: payslipData.pf}
										</Text>
									...
									<Box w='25%'>
										...
										<Text>
											{payslipData.account_no === 'null' || payslipData.account_no === 'undefined'
												? 'No Account Number'
