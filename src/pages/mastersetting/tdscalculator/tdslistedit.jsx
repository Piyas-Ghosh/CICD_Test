...
const TdsListEdit = () => {
		...
		const departmentList = async () => {
			try {
				...
				const response1 = await fetch(
					`${import.meta.env.VITE_API_URL}/declaration-list`,
					{
						method: 'GET',
						headers: {
				...
				const response2 = await fetch(
					`${import.meta.env.VITE_API_URL}/tds-slab`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
	...
	const ActionTemplate = ({ rowData, tdsSlab }) => {
		...
		const tierUpdate = async (e) => {
			...
			try {
				const response2 = await fetch(
					`${import.meta.env.VITE_API_URL}/declaration-exempt/${userId}`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
		...
		const tdsSubmit = async (e) => {
			...
			try {
				...
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/exempt-submit`,
					{
						method: 'POST',
						body: formData,
						headers: {
