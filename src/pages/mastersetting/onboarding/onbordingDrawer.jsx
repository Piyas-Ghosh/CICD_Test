...
const OnbordingDrawer = ({ rowData, fromValue, empUser, setRequestStatus }) => {
			...
			const formDataValue = async () => {
				try {
					const response1 = await fetch(
						`${import.meta.env.VITE_API_URL}/emp-cost-center/${formData.department}`,
						{
							method: 'GET',
							headers: {
	...
	useEffect(() => {
		if (formData?.employee_code_prefix?.length) {
			const formDataValue = async () => {
				...
				try {
					const response1 = await fetch(
						`${import.meta.env.VITE_API_URL}/emp-emp_code`,
						{
							method: 'POST',
							body: fromValues,
							headers: {
	...
	const stateMaster = async () => {
		try {
			const response1 = await fetch(
				`${import.meta.env.VITE_API_URL}/state-master-employee`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const formDataValue = async (e) => {
		...
		const response = await fetch(
			`${import.meta.env.VITE_API_URL}/emp-details/${id}`,
			{
				method: 'GET',
				headers: {
					Authorization: `Bearer ${token}`,
	...
	const offerSend = async (e) => {
		...
		try {
			setIsLoading(true);
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/send-offerletter`,
				{
					method: 'POST',
					body: fromValues,
					headers: {
	...
	const convertEmployee = async (e) => {
		...
		try {
			setIsLoading(true);
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/convert-to-employee`,
				{
					method: 'POST',
					body: fromValues,
					headers: {
