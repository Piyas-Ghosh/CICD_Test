		...
		const departmentList = async () => {
			try {
				...
				const response1 = await fetch(
					`${import.meta.env.VITE_API_URL}/asset-list/${rows}?page=${first}`,
					{
						method: 'GET',
						headers: {
				...
				const response2 = await fetch(
					`${import.meta.env.VITE_API_URL}/emp-list`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
				...
				const response3 = await fetch(
					`${import.meta.env.VITE_API_URL}/uom-master`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
			...
			if (hasEmptySpecification) {
				...
				try {
					...
					const response = await fetch(
						`${import.meta.env.VITE_API_URL}/asset-update`,
						{
							method: 'POST',
							body: formData,
							headers: {
	...
	const ActionTemplate = (rowData) => {
		...
		const assectAssign = async (e) => {
			...
			try {
				setIsLoading(true);
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/asset-assign-post`,
					{
						method: 'POST',
						body: formData,
						headers: {
