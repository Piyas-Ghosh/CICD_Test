		...
		const formDataValue = async () => {
			try {
				...
				const response1 = await fetch(
					`${import.meta.env.VITE_API_URL}/emp-list/${rows}?page=${first}`,
					{
						method: 'GET',
						headers: {
	...
	const ActionTemplate = (rowData) => {
		...
		const empBlock = async (e) => {
			...
			const formDataValue = async () => {
				try {
					const response1 = await fetch(
						`${import.meta.env.VITE_API_URL}/emp-block/${empId}`,
						{
							method: 'GET',
							headers: {
								Authorization: `Bearer ${token}`,
		...
		const empUnBlock = async (e) => {
			...
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/emp-unblock/${empId}`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${token}`,
		...
		const formDataValue = async (e) => {
			...
			try {
				setFromLoader(true);
				const response1 = await fetch(
					`${import.meta.env.VITE_API_URL}/emp-form-view`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
				...
				const response2 = await fetch(
					`${import.meta.env.VITE_API_URL}/emp-details/${empId}`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
		...
		const sendTermination = async (e) => {
			...
			try {
				...
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/send-terminationletter`,
					{
						method: 'POST',
						body: formValues,
						headers: {
		...
		const sendWarning = async (e) => {
			...
			try {
				setIsLoadingModal(true);
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/send-warningletter`,
					{
						method: 'POST',
						body: formValues,
						headers: {
		...
		const sendAppointment = async (e) => {
			...
			try {
				setIsLoadingModal(true);
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/send-appointmentletter`,
					{
						method: 'POST',
						body: formValues,
						headers: {
		...
		const clearId = async (e) => {
			...
			try {
				setIsLoadingModal(true);
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/clear-device-id/${userId}`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
