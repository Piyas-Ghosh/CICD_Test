		...
		const formDataValue = async () => {
			try {
				...
				const response1 = await fetch(
					`${import.meta.env.VITE_API_URL}/probation-employee`,
					{
						method: 'GET',
						headers: {
	...
	const ActionTemplate = (rowData) => {
		...
		const probationToPermanent = async () => {
			try {
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/convert-to-permanat-employee/${userId}`,
					{
						method: 'GET',
						headers: {
							Authorization: `Bearer ${token}`,
		...
		const sendConfirmationletter = async (e) => {
			...
			try {
				...
				const response = await fetch(
					`${import.meta.env.VITE_API_URL}/send-confirmationletter`,
					{
						method: 'POST',
						body: formValues,
						headers: {
